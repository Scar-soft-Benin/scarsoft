...
const filterOptions = [
  { key: 'all', label: 'Afficher Tout', icon: 'pi pi-th-large' },
  { key: 'web', label: 'Sites Web', icon: 'pi pi-globe' },
  { key: 'mobile', label: 'Applications Mobile', icon: 'pi pi-mobile' },
  { key: 'ui-ux', label: 'Conception UI/UX', icon: 'pi pi-palette' }
];
...
const NosProject = () => {
  ...
  const handleFilterChange = (filterKey: string) => {
    ...
    if (filterKey === 'all') {
      ...
      const filtered = projects.filter(project => project.category === filterKey);
      setFilteredProjects(filtered);
    }
  };

  // Animation GSAP (basée sur le code initial)
  useEffect(() => {
  ...
  }, [filteredProjects]); // Re-animer quand les projets changent
    ...
    <div className="py-8 px-6">
      ...
      <div className="text-center mb-6">
        <p className="text-gray-600">
          ...
          {activeFilter !== 'all' && (
            <span className="ml-1">
              dans {filterOptions.find(opt => opt.key === activeFilter)?.label.toLowerCase()}
            </span>
          )}
        </p>
      </div>
        ...
        {filteredProjects.map((project, index) => (
          ...
          >
            <img
              ...
              loading="lazy"
              onLoad={() => console.log('Image chargée:', project.image)}
              onError={(e) => {
                console.error('Erreur de chargement image:', project.image);
